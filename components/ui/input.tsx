...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      ...
      data-slot='input'
      className={cn(
        'flex h-12 w-full rounded-md border border-[var(--input-border)] bg-white px-3 text-sm sm:text-[16px] text-[var(--foreground)] placeholder:text-sm sm:placeholder:text-[16px] outline-none transition-colors',
